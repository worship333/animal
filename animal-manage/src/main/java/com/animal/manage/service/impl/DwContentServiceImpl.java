...
public class DwContentServiceImpl implements DwContentService {
	...
	public EasyUIDataGrid showContent(long categoryId, int page, int rows) {
		return dwContentDubboServiceImpl.selContentByPage(categoryId, page, rows);
	}

	@Override
	public int save(DwContent content) {
		Date date = new Date();
		content.setCreated(date);
		content.setUpdated(date);
		
		int index = dwContentDubboServiceImpl.insContent(content);
